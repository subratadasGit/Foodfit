...
const fetchDashboardData = async () => {
    try {
        const [recipesRes, pantryRes, recentRes] = await Promise.all([
        ...
        setStats({
            totalRecipes: recipesRes?.data?.data?.stats?.total_recipes ?? recipesRes?.data?.data?.stats?.totalRecipes ?? 0,
            pantryItems: pantryRes?.data?.data?.stats?.total_items ?? pantryRes?.data?.data?.stats?.totalItems ?? 0,
            mealsThisWeek: 0 // No API yet
        });

        setRecentRecipes(recentRes?.data?.data?.recipes || []);
        setUpcomingMeals([]); // No API yet
